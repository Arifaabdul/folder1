...
def summ(Sample_List):
    sum1=sum(Sample_List)   
    return sum1
summ(Sample_List)
...
def amazon_offer(amount):
    if amount >1000:
        offer=amount-(amount*0.25)
        return offer
    elif amount <500:
        offer=amount-(amount*0.5)
        return offer
    elif amount >=500 or amount <=100:
        offer=amount-(amount*0.20)
        return offer
    else:
...
def myfunction(sample_list):
    uniq_val={1,2,3,3,3,3,4,5}
    return uniq_val
myfunction(sample_list)
